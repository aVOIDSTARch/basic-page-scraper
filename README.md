...
- **Lightning Fast** — Built on [Bun](https://bun.sh) for blazing performance
- **Smart Media Detection** — Automatically extracts images, videos, audio, fonts, and documents
- **SHA-256 Deduplication** — Prevents duplicate downloads across scrapes
- **Size-Aware Downloads** — Configurable per-file limits with oversized content tracking
- **Rich Manifests** — JSON manifests with URLs, MIME types, file hashes, and sizes
...
```
output/
└── example.com-1702656000000/
    ├── index.html          # Fetched HTML
    ├── manifest.json       # Media catalog with hashes
    ├── metadata.json       # Scrape metadata
    ├── summary.scrape      # Human-readable summary
    ├── oversized_content.json  # Files that exceeded size limits
...
```
scrapr/
├── scrapr              # CLI launcher (bash)
├── scripts/
│   └── install.sh      # Runtime installer
└── scraper/
    ├── cli.ts          # Main CLI entry point
    ├── utils.ts        # URL parsing, media extraction, hashing
